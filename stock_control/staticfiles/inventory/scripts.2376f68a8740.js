document.addEventListener("DOMContentLoaded", function () {
    ...
    // --- Update Full Items Withdrawal based on Parts Withdrawn ---
    const partsInput = document.getElementById("id_parts_withdrawn");
    const fullItemsInput = document.getElementById("id_quantity");
    const unitsPerQuantityInput = document.getElementById("id_units_per_quantity");

    // This function recalculates full items from the parts entered.
    function updateFullItems() {
        let partsValue = parseInt(partsInput.value, 10) || 0;
        let threshold = parseInt(unitsPerQuantityInput.value, 10) || 1; // default to 1 if not set

        // If parts are less than threshold, full items are 0
        if (partsValue < threshold) {
            fullItemsInput.value = 0;
        } else {
            // Calculate how many full items can be made from the parts
            let fullItems = Math.floor(partsValue / threshold);
            fullItemsInput.value = fullItems;
        }
    }

    // Listen for changes on the parts input
    if (partsInput) {
        partsInput.addEventListener("input", updateFullItems);
    }

    // Optionally, update on page load in case the field is pre-filled
    updateFullItems();
