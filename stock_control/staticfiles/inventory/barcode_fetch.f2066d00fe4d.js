document.addEventListener("DOMContentLoaded", function () {
    ...
    barcodeInput.addEventListener("change", async function () {
        ...
        try {
            ...
            const response = await fetch(`/parse-barcode/?raw=${encodeURIComponent(rawBarcode)}`);
            ...
            if (data.product_code) {
                try {
                    const productResponse = await fetch(`/get-product-by-barcode/?barcode=${data.product_code}`);
