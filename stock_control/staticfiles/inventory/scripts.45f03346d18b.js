document.addEventListener("DOMContentLoaded", function () {
    ...
    // Autofocus barcode if default is "barcode"
    if (barcodeInput) {
        ...
        try {
            ...
            if (data.product_feature === "volume") {
                ...
                if (volumeSection) {
                    ...
                    // Build increments in the dropdown
                    if (volumeSelect) {
                        volumeSelect.innerHTML = "";
                        let totalVol = parseFloat(data.stock) || 0;
                        let step = 50;  // e.g. step of 50 mL; adjust as desired
                        let current = 0;
                ...
                // Hide volume section if unit-based
                if (volumeSection) volumeSection.style.display = "none";
            }
        } catch (err) {
    ...
    // --- 4) Manual Entry: Fetch Product Details on Dropdown change ---
    ...
    const manualStockDisplay = document.getElementById("manual-stock-display");
    const manualUnitsDisplay = document.getElementById("manual-units-display");

    if (productDropdown) {
        productDropdown.addEventListener("change", function () {
            const productId = this.value;
            if (productId) {
                fetch(`/get-product-by-id/?product_id=${productId}`)
                    .then(res => res.json())
                    .then(data => {
                        if (data.error) {
                            barcodeManual.value = "";
                            manualStockDisplay.textContent = "";
                            manualUnitsDisplay.textContent = "";
                            unitsPerQuantityInput.value = "";
                            if (volumeSection) volumeSection.style.display = "none";
                        } else {
                            barcodeManual.value = data.barcode || "";
                            manualStockDisplay.textContent = data.stock ?? "";
                            manualUnitsDisplay.textContent = data.units_per_quantity ?? "";
                            unitsPerQuantityInput.value = data.units_per_quantity ?? "";
    ...
    function updatePartCalculations() {
        if (!partsInput || !unitsPerQuantityInput) return;
        const parts = parseInt(partsInput.value, 10) || 0;
        const threshold = parseInt(unitsPerQuantityInput.value, 10) || 1;
        ...
        if (partsRemainingInput) {
            if (remainder === 0 && parts > 0) {
                partsRemainingInput.value = 0;
            } else {
                partsRemainingInput.value = threshold - remainder;
            }
        }
    }
    ...
    // --- 7) Table Search Feature for Tracking Withdrawals ---
    ...
    if (searchInput && tableBody) {
        searchInput.addEventListener("keyup", function () {
            ...
            for (let row of rows) {
                ...
                const usernameText = row.cells[7]?.textContent.toLowerCase() || "";
                if (
