...
// 8) ANALYSIS CHARTS
    // Ensure data is available in the template
    const productNames = JSON.parse(document.getElementById("stockLevelsChart").dataset.labels);
    const currentStock = JSON.parse(document.getElementById("stockLevelsChart").dataset.stock);
    const stockThresholds = JSON.parse(document.getElementById("stockLevelsChart").dataset.thresholds);

    const dateLabels = JSON.parse(document.getElementById("withdrawalsOverTimeChart").dataset.dates);
    const withdrawalCounts = JSON.parse(document.getElementById("withdrawalsOverTimeChart").dataset.withdrawals);

    const topProducts = JSON.parse(document.getElementById("topWithdrawnProductsChart").dataset.topproducts);
    const topWithdrawnCounts = JSON.parse(document.getElementById("topWithdrawnProductsChart").dataset.topcounts);
    ...
    const withdrawalsOverTimeCtx = document.getElementById("withdrawalsOverTimeChart").getContext("2d");
    new Chart(withdrawalsOverTimeCtx, {
        type: "line",
        data: {
            labels: dateLabels,
            datasets: [{
    ...
    const topWithdrawnProductsCtx = document.getElementById("topWithdrawnProductsChart").getContext("2d");
    new Chart(topWithdrawnProductsCtx, {
        type: "bar",
        data: {
            labels: topProducts,
            datasets: [{
                label: "Quantity Withdrawn",
                data: topWithdrawnCounts,
