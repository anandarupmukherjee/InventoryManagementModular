document.addEventListener("DOMContentLoaded", function () {
    ...
    if (reportForm) {
        ...
        // }
        function getForecastData(canvasId) {
            const element = document.getElementById(canvasId);
            return {
                labels: JSON.parse(element.getAttribute("data-labels") || "[]"),
                withdrawals: JSON.parse(element.getAttribute("data-withdrawals") || "[]"),
        ...
        // ✅ Render Withdrawal Forecasting Chart
        const forecastData = getForecastData("withdrawalForecastChart");
        new Chart(document.getElementById("withdrawalForecastChart").getContext("2d"), {
            type: "line",
            data: {
                labels: [...forecastData.labels, ...forecastData.forecastDates],
                datasets: [
                    { label: "Actual Withdrawals", data: forecastData.withdrawals, borderColor: "blue", fill: false },
                    ...
                    { label: "Forecast (Next 7 Days)", data: [...Array(forecastData.withdrawals.length).fill(null), ...forecastData.forecastValues], borderColor: "green", borderDash: [5, 5], fill: false }
                ]
            }
        });
